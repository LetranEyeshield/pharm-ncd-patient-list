...
import { saveVitamins } from "@/app/lib/api";
import { useState } from "react";
import { vitsCardList } from "../constants/lists";
import { VitaminsType } from "../types/Vitamins";
import Banner from "../components/Banner";
import toast from "react-hot-toast";

export default function MedsCardForm() {
  const [form, setForm] = useState<VitaminsType>({
    _id: "",
    cardName: "",
    cardDate: new Date(),
    initialStock: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await saveVitamins(form);
      if (res.success) {
        //alert(res.message);
