...
import { saveMaintanance } from "@/app/lib/api";
import { useState } from "react";
import { maintenanceCardList } from "../constants/lists";
import { MaintenanceType } from "../types/Maintenance";
import Banner from "../components/Banner";
import toast from "react-hot-toast";

export default function MedsCardForm() {
  const [form, setForm] = useState<MaintenanceType>({
    _id: "",
    cardName: "",
    cardDate: new Date(),
    initialStock: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await saveMaintanance(form);
      if (res.success) {
        //alert(res.message);
